...
def main(argv):
    ...
    """     -------------------------------------   Query arguments ---------------------------------     """
    query_args.add_argument("-qk", "--query-key", action='store', dest='query_key', required=False, help='Query given comma separated registry keys, example: -qk "Software\\Microsoft\\Windows\\CurrentVersion\\Run" (all registry values from queried key(s) would be printed)')
    query_args.add_argument("-qv", "--query-value", action='store', dest='query_value', required=False, help='Query given comma separated registry values, example: -qv "Software\\Microsoft\\Windows\\CurrentVersion\\Run\\ctfmon.exe" (only the queried value would be printed)')
    query_args.add_argument("-qs", "--query-subkeys", action='store_true', dest='query_subkeys', required=False,
                            default=False, help='Display root sub keys of given registry hive (Default: False)')

    """     -------------------------------------   Search arguments ---------------------------------     """
    search_args.add_argument("-tm", "--time-modified-string", action='store', dest='key_modified', required=False, help='Comma separated regex date patterns: -tm "2014-09-04 09:50",”2013|2014” would search for all keys modified in 2014-09-04 at 09:50 and on 2013 or 2014')
    search_args.add_argument("-td", "--time-modified-datetime", action='store', dest='key_modified_dt', required=False,
                             help='Comma separated Date time patterns: [datetime]..[datetime] or [Operator][datetime], where opertor is one of: ">", "<". Example: -td "2014-09-04 13:12:19..2014-09-04 13:12:25.703125",">2014-09-04"')
    search_args.add_argument("-ks", "--key-string", action='store', dest='key_string', required=False,
                             help='A string value, example: -ks "\\Run" would search for all keys having given string anywhere in their key path string')
    search_args.add_argument("-kr", "--key-regex", action='store', dest='key_regex', required=False,
                             help='A regex string, example: -kr "\\\\Run$" would search for all keys which ends with "\Run" string')
    search_args.add_argument("-kb", "--key-bin", action='store', dest='key_bin', required=False,
                             help='A binary value expressed in UTF8 escaped hex string, example: -kb "\\x5C\\x52\\x75\\x6E" would search for all keys having given sequence of bytes anywhere their key path bytes')
    search_args.add_argument("-vs", "--value-string", action='store', dest='value_string', required=False,
                             help='A string value, example: -vs "ctfmon" would search for all values having given string anywhere in their value name')
    search_args.add_argument("-vr", "--value-regex", action='store', dest='value_regex', required=False,
                             help='A regex string, example: -vr "ctfmon\.exe$" would search for value name which ends with "ctfmon.exe" string')
    search_args.add_argument("-vb", "--value-bin", action='store', dest='value_bin', required=False,
                             help='A binary value expressed in UTF8 escaped hex string, example: -vb "\\x63\\x74\\x66\\x6D\\x6F\\x6E" would search for all values having given sequence of bytes anywhere their name bytes')
    search_args.add_argument("-ds", "--data-string", action='store', dest='data_string', required=False,
                             help='A string value, example: -ds "CTF" would search for all values having given string anywhere in their data value')
    search_args.add_argument("-dr", "--data-regex", action='store', dest='data_regex', required=False,
                             help='A regex string, example: -dr "^CTF" would search for all values which starts with "CTF" string')
    search_args.add_argument("-db", "--data-bin", action='store', dest='data_bin', required=False,
                             help='A binary value expressed in UTF8 escaped hex string, example: -db "\\x43\\x54\\x46" would search for all values having given sequence of bytes anywhere their data bytes')
    ...
    output_args.add_argument("-o", "--output-file", type=str, action='store', dest='output_file', required=False,
                             default="", help='Filepath to output file (Default: stdout)')

    output_args.add_argument("-a", "--output-format-all", action='store_true', dest='output_format_all', required=False,
                             default=False, help='Output all format fields [Time consuming task]')
    ...
    plugin_args.add_argument("-p", "--plugins", action='store', dest='plugins_to_execute', required=False,
                             help='Specify plugins to run with their parameters: ["autoruns -d","services"')
